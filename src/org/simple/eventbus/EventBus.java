...
public final class EventBus {
    ...
    public void post(String tag, Object... event) {
        if (event == null || event.length == 0) {
            mLocalEvents.get().offer(new EventType(tag, new NULL().getClass()));
            mDispatcher.dispatchEvents(new NULL());
        } else {
            ...
            mLocalEvents.get().offer(new EventType(tag, c));
    ...
    public void postSticky(Object... event) {
        postSticky(EventType.DEFAULT_TAG, event);
    }
    ...
    public void removeStickyEvent(Class<?>... eventClass) {
        removeStickyEvent(EventType.DEFAULT_TAG, eventClass);
    }
     ...
     * @param eventClass
     */
    public void removeStickyEvent(String tag, Class<?>... eventClass) {
        EventType other;
        if (eventClass == null || eventClass.length == 0) {
            other = new EventType(tag, new NULL().getClass());
        } else {
    ...
    private class EventDispatcher {
        ...
        private void handleEvent(EventType eventType, Object... aEvent) {
            List<Subscription> subscriptions = loadMapKey(mSubcriberMap, eventType);
        ...
        private List<Subscription> loadMapKey(Map<EventType, CopyOnWriteArrayList<Subscription>> mSubcriberMap, EventType eventType) {
            List<Subscription> result = new ArrayList<Subscription>();
            for (EventType key : mSubcriberMap.keySet()) {
                if (key.equals(eventType)) {
                    result.addAll(mSubcriberMap.get(key));
                }
            }
            return result;
        }
